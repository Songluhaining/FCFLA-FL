...
def write_header_in_result_file(row, sheet):
    ...
    sheet.write(row, VARCOP_RANK_COL, FCFLA_RANK)
    sheet.write(row, VARCOP_EXAM_COL, FCFLA_EXAM)
    sheet.write(row, VARCOP_SPACE_COL, FCFLA_SPACE)

    sheet.write(row, VARCOP_TC_SLICED_RANK_COL, FCFLA_TC_RANK)
    sheet.write(row, VARCOP_TC_SLICED_EXAM_COL, FCFLA_TC_EXAM)
    ...
    sheet.write(row, VARCOP_DISABLE_BPC_RANK_COL, FCFLA_DISABLE_BPC_RANK)
    sheet.write(row, VARCOP_DISABLE_BPC_EXAM_COL, FCFLA_DISABLE_BPC_EXAM)
...
def write_result_to_file(row, sheet, ranking_results, fb_results, search_spaces, is_var_bug):
    varcop_space = len(get_set_of_stms(search_spaces[SS_FCFLA]))
    ...
    for stm in ranking_results[FCFLA_RANK].keys():
        ...
        if is_var_bug:
            sheet.write(row, VARCOP_RANK_COL, ranking_results[FCFLA_RANK][stm][RANK])
            sheet.write(row, VARCOP_EXAM_COL, (ranking_results[FCFLA_RANK][stm][RANK] / all_stms) * 100)
        else:
            sheet.write(row, VARCOP_RANK_COL, ranking_results[FCFLA_DISABLE_BPC_RANK][stm][RANK])
            sheet.write(row, VARCOP_EXAM_COL, (ranking_results[FCFLA_DISABLE_BPC_RANK][stm][RANK] / all_stms) * 100)
            sheet.write(row, IS_VAR_BUG_COL, 0)

        sheet.write(row, VARCOP_SPACE_COL, varcop_space)

        sheet.write(row, VARCOP_TC_SLICED_RANK_COL, ranking_results[FCFLA_TC_RANK][stm][RANK])
        sheet.write(row, VARCOP_TC_SLICED_EXAM_COL, (ranking_results[FCFLA_TC_RANK][stm][RANK] / all_stms) * 100)
        ...
        sheet.write(row, VARCOP_DISABLE_BPC_RANK_COL, ranking_results[FCFLA_DISABLE_BPC_RANK][stm][RANK])
...
def get_suspicious_space(mutated_project_dir, filtering_coverage_rate, coverage_version):
    ...
    search_spaces = {SS_FCFLA: stms_isolated_by_varcop, SS_SLICING: tc_sliced_based_isolation,
...
def multiple_bugs_ranking(system_name, buggy_systems_folder, sbfl_metrics, alpha=0.5,
    ...
    if os.path.exists(buggy_systems_folder):
        ...
        experiment_file_name = join_path(aggregation_result_dir,
                                        system_name + "Myway_ranking_1Bug_fcfla.xlsx")

        wb = Workbook(experiment_file_name)

        for i in range(0, len(sbfl_metrics)):
            sheet.append(wb.add_worksheet(sbfl_metrics[i]))            #添加多张表
