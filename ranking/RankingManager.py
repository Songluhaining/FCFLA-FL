...
def get_set_of_stms_withScore(dict_of_stm_per_variant):
    stm_set = {}
    for variant in dict_of_stm_per_variant:
        for stm in dict_of_stm_per_variant[variant]:
            if stm not in stm_set:
...
def global_ranking_a_suspicious_list(all_stms_of_the_system, all_stms_in_failing_variants, suspicious_stms_list,
                                     ...
                                     spectrum_expression, aggregation_type, normalized_type, alpha=0, isFCFLA = False):      #关键点在于suspicious_stms_list
...
def product_based_assessment(mutated_project_dir, all_stms_in_failing_products, spectrum_expressions,
                             spectrum_coverage_prefix):
    list_of_stms = get_set_of_stms(all_stms_in_failing_products)     # 一个list，所有产品的语句
    failing_passing_variants_of_stms, total_fails, total_passes = get_num_passing_failing_variants(mutated_project_dir,
                                                                                                   list_of_stms,
                                                                                                   spectrum_coverage_prefix)  #关键一步
    variant_level_suspiciousness = {}
    for spectrum_expression in spectrum_expressions:   #30个排序指标
...
def fcfla(buggy_statements, local_scores, variant_level_suspiciousness, search_spaces,
    ...
    fcfla_isolated_set = get_set_of_stms(search_spaces[SS_FCFLA])
    ...
    for metric in spectrum_expressions:
        ...
        full_ranked_list = global_ranking_a_suspicious_list(search_spaces[SS_ALL_STMS],
                                                            ...
                                                            metric,
                                                            aggregation_type,
                                                            normalized_type, alpha, isFCFLA=False)

        all_buggy_positions[metric][FCFLA_DISABLE_BPC_RANK] = locate_multiple_bugs(buggy_statements,
                                                                                    stms_in_f_products_set,
                                                                                    full_ranked_list,
                                                                                    full_ranked_list)

        fcfla_isolated_ranked_list = global_ranking_a_suspicious_list(search_spaces[SS_ALL_STMS],
                                                                       search_spaces[SS_STMS_IN_F_PRODUCTS],
                                                                       search_spaces[SS_FCFLA],
                                                                       local_scores[metric],
                                                                       variant_level_suspiciousness[metric],
                                                                       metric,
                                                                       aggregation_type,
                                                                       normalized_type, alpha, isFCFLA=True)

        all_buggy_positions[metric][FCFLA_RANK] = locate_multiple_bugs(buggy_statements,
                                                                        fcfla_isolated_set,
                                                                        fcfla_isolated_ranked_list,
                                                                        full_ranked_list)
        ...
        all_buggy_positions[metric][FCFLA_TC_RANK] = locate_multiple_bugs(buggy_statements,
                                                                          sliced_isolated_set,
...
def ranking_multiple_bugs(buggy_statements, mutated_project_dir, search_spaces, spectrum_expressions,
    ...
    local_suspiciousness_of_all_the_system = local_ranking_a_suspicious_list(mutated_project_dir,
                                                                             search_spaces[SS_STMS_IN_F_PRODUCTS],
                                                                             spectrum_expressions,
                                                                             spectrum_coverage_prefix)          #能够得到每个语句的可疑分数
    sbfl(buggy_statements, mutated_project_dir, search_spaces, spectrum_expressions, spectrum_coverage_prefix,
         coverage_rate)
    fcfla(buggy_statements, local_suspiciousness_of_all_the_system, variant_level_suspiciousness, search_spaces,
           spectrum_expressions, aggregation_type, normalized_type, alpha)

    fcfla_ranking_time = time.time() - start_time
    return all_buggy_positions, fcfla_ranking_time
    ...
    for variant in all_statements_in_failing_variants:
        ...
        if (len(all_statements_in_failing_variants[variant]) > 0):
            ...
            for stm in all_suspicious_stm:
                # local_score = local_suspiciousness_of_all_the_system[variant][i][1]; num_of_failing_test = local_suspiciousness_of_all_the_system[variant][i][2]
                local_score, num_of_failing_test = get_local_score(stm, local_suspiciousness_of_all_the_system[variant])  #重点，计算每条语句的分数
...
def global_score_aggregation_arithmetic_mean(all_stms_of_the_system, normalized_score_list,
                                             variant_level_suspiciousness, spectrum_expression, suspicious_stms_list, isFCFLA, alpha=0):
    score_type = spectrum_expression + "_score"
    all_stms_score_list = {}
    for variant in normalized_score_list:
        for stm in normalized_score_list[variant]:
    ...
    if isFCFLA:
        return fcfla_ranking(all_stms_score_list, variant_level_suspiciousness, spectrum_expression,
                             suspicious_stms_list, alpha)
    return varcop_ranking(all_stms_score_list, variant_level_suspiciousness, spectrum_expression,
                          alpha)  # 根据score_type从大到小排序
...
def get_executed_stms_of_the_system(mutated_project_dir, spectrum_coverage_prefix, coverage_rate):
    ...
    for variant in variants_list:
        ...
        failed_file = join_path(test_coverage_dir, spectrum_failed_file)   #错误的频谱文件
...
def fcfla_ranking(statements_infor, variant_level_suspiciousness, spectrum_expression, suspicious_stms_list, alpha=0.5):
    ranked_list = []
    suspicious_stms_withscore = get_set_of_stms_withScore(suspicious_stms_list)
    fusion_a = {}
    fusion_b = {}
    score_type = spectrum_expression + "_score"
    for (key, value) in statements_infor.items():
...
def descending_sort_fcfla(data, sorted_element, data2):
    for i in range(0, len(data) - 1):
        for j in range(i + 1, len(data)):
            if data2[i] < data2[j]:
                data[i], data[j] = \
